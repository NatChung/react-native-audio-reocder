...
public final class UlawEncoderInputStream extends InputStream
{
                              ...
                              byte[] ulawBuf, int ulawOffset, int length, int max)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            if (pcmOffset < 1600)
            {
                pcm = (0xff & pcmBuf[pcmOffset++]) + (pcmBuf[pcmOffset++] << 8);
     ...
     *
     * @param pcmBuf
     *            array containing 16 bit pcm data.
     * @param offset
     *            offset of start of 16 bit pcm data.
     * @param length
     *            number of pcm samples (not number of input bytes)
    ...
    public static int maxAbsPcm(byte[] pcmBuf, int offset, int length)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            if (offset < 1600)
            {
                pcm = (0xff & pcmBuf[offset++]) + (pcmBuf[offset++] << 8);
            }
            if (pcm < 0)
                pcm = -pcm;
            if (pcm > max)
     ...
     * @param max
     *            pcm value corresponding to maximum ulaw value.
     */
